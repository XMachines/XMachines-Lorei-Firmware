...
inline void gcode_M106() {
  uint16_t s = code_seen('S') ? code_value_short() : 255,
           p = code_seen('P') ? code_value_short() : -1; //<-- Original false value was 0
  NOMORE(s, 255);

  if (p == -1) { //<-- Added to turn all fans to the same setting if no p is specified
    for (int i = 0; i < FAN_COUNT; i++)
      fanSpeeds[i] = s;
  }
  else if (p < FAN_COUNT) fanSpeeds[p] = s;
}
...
inline void gcode_M107() {
  uint16_t p = code_seen('P') ? code_value_short() : -1; //<-- Original false value was 0

  if (p == -1) { //<-- Added to turn all fans to the same setting if no p is specified
    for (int i = 0; i < FAN_COUNT; i++)
      fanSpeeds[i] = 0;
  }
  else if (p < FAN_COUNT) fanSpeeds[p] = 0;
}
